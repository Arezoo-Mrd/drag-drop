...
const DrawingBoard: React.FC = () => {
 ...
 const [strokeColor, setStrokeColor] = useState("#6a7282");
 const [fillColor, setFillColor] = useState("#6a7282");
 ...
 const handleMouseDown = (e: any) => {
  ...
  if (selectedTool === "text") {
   setShapes([
    ...shapes,
    { type: "text", x: pos.x, y: pos.y, text: textInput, fill: strokeColor },
   ]);
   return;
  }
  ...
  if (selectedTool === "free" || selectedTool === "eraser") {
   setCurrentShape({
    type: selectedTool,
    points: [pos.x, pos.y],
    stroke: selectedTool === "eraser" ? "white" : strokeColor,
   });
 ...
 const handleMouseMove = (e: any) => {
  ...
  if (selectedTool === "free" || selectedTool === "eraser") {
   ...
   setCurrentShape((prevShape) => ({
    ...prevShape!,
    points: [
     prevShape?.points?.[0] || 0,
     prevShape?.points?.[1] || 0,
     pos.x,
     pos.y,
    ],
   }));
 ...
 const handleMouseUp = () => {
  ...
  if (currentShape) {
   if (selectedTool === "circle" && currentShape.width && currentShape.height) {
    const radius =
     Math.sqrt(
      Math.pow(currentShape.width, 2) + Math.pow(currentShape.height, 2)
     ) / 2;
 ...
 const normalizeShape = (shape: Shape): Shape => {
  if (shape.type === "rectangle" || shape.type === "circle") {
   ...
   const x =
    shape.width && shape.width < 0 ? (shape.x || 0) + shape.width : shape.x;
   const y =
    shape.height && shape.height < 0 ? (shape.y || 0) + shape.height : shape.y;
 ...
 const renderShape = (shape: Shape, key: number) => {
  ...
  switch (normalizedShape.type) {
   case "rectangle":
    if (normalizedShape.width && normalizedShape.height) {
     const cornerRadius = Math.min(
      5,
      normalizedShape.width / 10,
      normalizedShape.height / 10
     );
     return (
      <Rect
       key={key}
   ...
   case "circle":
    if (
     normalizedShape.x !== undefined &&
     normalizedShape.y !== undefined &&
     normalizedShape.radius
    ) {
     return (
      <Circle
       key={key}
